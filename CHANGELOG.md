...
- `doctor` subcommand — reports ledger/gavel-cache/chisel-recovery size and entry counts, whether `ashlar` resolves on `PATH`, and (if `hooks/posttooluse_bash_chisel.py` exists alongside the checkout) prints the exact `.claude/settings.json` snippet to wire the transparent hook by hand, for anyone using the standalone CLI/binary instead of the plugin installer.
